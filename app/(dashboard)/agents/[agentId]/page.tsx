...
export default function AgentDetailPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // Fetch versions
      const versionsRes = await fetch(`/api/agent/versions/${agentId}`);
      ...
      // Fetch deployments
      const deploymentsRes = await fetch(`/api/agent/deployments/${agentId}`);
  ...
  const handleCreateVersion = async () => {
    ...
    try {
      const res = await fetch(`/api/agent/versions/${agentId}`, {
        method: "POST",
  ...
  const handleDeploy = async () => {
    ...
    setDeploying(true);
    try {
      const res = await fetch("/api/agent/deployments", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          agentId,
  ...
  const handlePromote = async (deploymentId: string, targetEnv: "staging" | "prod") => {
    ...
    try {
      const res = await fetch("/api/agent/deployments/promote", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
  ...
  const handleRollback = async () => {
    ...
    setRollingBack(true);
    try {
      const res = await fetch("/api/agent/deployments/rollback", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          agentId,
